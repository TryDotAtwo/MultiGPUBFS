...
extern "C" int mgbfs_route_query(uint32_t capacity,MgbfsRouteBytes* out){
  if(!out)return 1;*out={};
  if(capacity==0||capacity>INT32_MAX)return 1;
  size_t sort_bytes=0,select_bytes=0;
  if(cub::DeviceRadixSort::SortPairs(nullptr,sort_bytes,static_cast<const Key128*>(nullptr),static_cast<Key128*>(nullptr),static_cast<const uint64_t*>(nullptr),static_cast<uint64_t*>(nullptr),int(capacity),Decompose{},0,128)!=cudaSuccess)return 2;
  if(cub::DeviceSelect::Flagged(nullptr,select_bytes,static_cast<uint32_t*>(nullptr),static_cast<uint8_t*>(nullptr),static_cast<uint32_t*>(nullptr),static_cast<uint32_t*>(nullptr),int(capacity))!=cudaSuccess)return 2;
  if(!sort_bytes||!select_bytes)return 2;
  const uint64_t c=capacity;
  *out={c*16,c*8,c*4,c*4,c,std::max(sort_bytes,select_bytes),sort_bytes,select_bytes};
  return 0;
}
...
extern "C" int mgbfs_route_create(uint32_t capacity,void** out,char* error,size_t error_capacity){
  ...
  try{
    MgbfsRouteBytes q{};
    if(mgbfs_route_query(capacity,&q)!=0)throw std::runtime_error("ROUTE_QUERY");
    auto p=std::make_unique<RoutePlan>();p->capacity=capacity;
    checked(cudaMalloc(&p->sorted,q.sorted));checked(cudaMalloc(&p->refs,q.refs));
    checked(cudaMalloc(&p->indices,q.indices));checked(cudaMalloc(&p->selected,q.selected));checked(cudaMalloc(&p->flags,q.flags));
    p->scratch_bytes=q.scratch;checked(cudaMalloc(&p->scratch,p->scratch_bytes));
