...
struct MgbfsMacroFrameLayout {
  uint64_t hash_bytes, ref_offset, ref_bytes;
  uint64_t state_offset, state_bytes, total_bytes;
};

inline int mgbfs_macro_frame_layout(uint32_t count, uint32_t stride,
                                   MgbfsMacroFrameLayout* out) {
  if (!out) return 1;
  *out = {};
  if (!stride || stride % 16) return 1;
  MgbfsMacroFrameLayout x{};
  x.hash_bytes = uint64_t(count) * 16;
  x.ref_bytes = uint64_t(count) * 16;
  x.state_bytes = uint64_t(count) * stride;
  x.ref_offset = (x.hash_bytes + 255) & ~uint64_t(255);
  x.state_offset = x.ref_offset + ((x.ref_bytes + 255) & ~uint64_t(255));
...
MGBFS_FRAME_HD inline bool mgbfs_macro_frame_word(
    const MgbfsMacroFrameLayout& layout, uint32_t stride, uint32_t begin,
    uint32_t source_count, uint32_t source_depth, uint32_t weight,
    const uint32_t* hashes, const uint64_t* refs, const uint32_t* states,
    uint64_t word, uint32_t* value) {
  if (word >= layout.total_bytes / 4) return false;
  const uint64_t byte = word * 4;
  if (byte < layout.hash_bytes) {
    *value = hashes[uint64_t(begin) * 4 + word];
  } else if (byte >= layout.ref_offset &&
             byte < layout.ref_offset + layout.ref_bytes) {
    const uint64_t row_word = (byte - layout.ref_offset) / 4;
    const uint64_t ref = refs[uint64_t(begin) + row_word / 4];
    if (ref >= source_count) return false;
    switch (row_word % 4) {
      case 0: *value = source_depth; break;
      case 1: *value = weight; break;
      case 2: *value = uint32_t(ref); break;
      default: *value = uint32_t(ref >> 32); break;
    }
  } else if (byte >= layout.state_offset &&
             byte < layout.state_offset + layout.state_bytes) {
    const uint64_t state_word = (byte - layout.state_offset) / 4;
