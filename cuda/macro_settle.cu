...
static_assert(sizeof(MgbfsFrontierState)==8 && offsetof(MgbfsFrontierState,count)==0 &&
              offsetof(MgbfsFrontierState,fatal)==4);
...
__global__ void begin(const uint32_t* count,const uint32_t* input_fatal,const uint32_t* history_counts,uint32_t candidates,uint32_t layers,uint32_t history_capacity,
  MgbfsMacroSettleState* state,uint32_t* output,uint64_t epoch){
  *output=0;if(state->fatal)return;
  if(input_fatal&&*input_fatal){state->fatal=4;return;}
...
static int settle_impl(void* raw,const void* future,const uint64_t* refs,const uint32_t* count,const uint32_t* input_fatal,const void* history,const uint32_t* history_counts,
  ...
  begin<<<1,1,0,stream>>>(count,input_fatal,history_counts,p->candidates,p->layers,p->history_capacity,state,survivor_count,epoch);
...
extern "C" int mgbfs_macro_settle_run(void* raw,const void* future,const uint64_t* refs,const uint32_t* count,const void* history,const uint32_t* history_counts,
  void* survivors,uint64_t* survivor_refs,uint32_t* survivor_count,MgbfsMacroSettleState* state,uint64_t epoch,void* raw_stream){
  return settle_impl(raw,future,refs,count,nullptr,history,history_counts,survivors,survivor_refs,survivor_count,state,epoch,raw_stream);
}
extern "C" int mgbfs_macro_settle_run_frontier(void* raw,const void* future,const uint64_t* refs,const MgbfsFrontierState* future_state,const void* history,const uint32_t* history_counts,
  void* survivors,uint64_t* survivor_refs,uint32_t* survivor_count,MgbfsMacroSettleState* state,uint64_t epoch,void* raw_stream){
  if(!future_state)return 1;
  return settle_impl(raw,future,refs,reinterpret_cast<const uint32_t*>(future_state),&future_state->fatal,
    history,history_counts,survivors,survivor_refs,survivor_count,state,epoch,raw_stream);
}
