...
fn bootstrap(path: &Path, rank: u32, world: u32) -> Result<[u8; 128]> {
    ...
    if rank == 0 {
        ...
        if unsafe { mgbfs_nccl_unique_id(id.as_mut_ptr().cast()) } != 0 {
            return Err("NCCL_UNIQUE_ID".into());
        }
        let mut bytes = Vec::with_capacity(140);
        bytes.extend_from_slice(MAGIC);
        bytes.extend_from_slice(&world.to_le_bytes());
        bytes.extend_from_slice(&id);
        let temporary = path.with_extension("rank0.tmp");
    ...
    loop {
        if let Ok(bytes) = std::fs::read(path) {
            if bytes.len() != 140
                || &bytes[..8] != MAGIC
                || u32::from_le_bytes(bytes[8..12].try_into().unwrap()) != world
            {
                return Err("BOOTSTRAP_FORMAT".into());
            }
            return Ok(bytes[12..].try_into().unwrap());
        }
...
fn run() -> Result<()> {
    ...
    let id = bootstrap(Path::new(&path), rank, world)?;
    let graph = MatrixGroup::unitriangular(4, 2)?;
    ...
    loop {
        ...
        let mut layer = bfs.snapshot()?;
        layer.sort();
        states.push(layer.iter().map(|x| hex(x)).collect::<Vec<_>>());
        if !bfs.advance()? {
            break;
        }
    }
    ...
    if let Some(directory) = std::env::args().nth(2) {
        ...
        std::fs::write(
            Path::new(&directory).join(format!("rank-{rank}.json")),
            &record,
        )
...
fn main() {
    if let Err(e) = run() {
        eprintln!("DISTRIBUTED_INCOMPLETE: {e}");
        std::process::exit(1)
    }
}
