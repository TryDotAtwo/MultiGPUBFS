...
fn shard_finalization_reuses_old_history_and_preserves_empty_shard_offsets() {
    unsafe {
        ...
        .enumerate()
        {
            assert_eq!(
                cudaMemcpy(
                    target.words[plane].cast_mut().cast(),
                    data.as_ptr().cast(),
                    16,
                    1
                ),
        ...
        for shard in 0..3 {
            let previous = KeysV1 {
                words: target.words.map(|p| p.add(shard)),
                rows: 1,
                reserved: 0,
            };
            owners.push(LibraryShard::new_window(previous, empty, 2, stream).unwrap());
        }
        for (shard, values) in [&[10u32, 20, 20, 21][..], &[11][..], &[12, 30, 30][..]]
            .iter()
            .enumerate()
        {
            let words: Vec<u32> = values.iter().flat_map(|&w| [1, 2, 3, w]).collect();
            assert_eq!(
                cudaMemcpy(input, words.as_ptr().cast(), words.len() * 4, 1),
                0
            );
        ...
        assert_eq!(
            finalize_shards(&mut owners, target, &mut ranges, stream).unwrap(),
            3
        );
        assert_eq!(ranges, [0..2, 2..2, 2..3]);
        assert_eq!(
            mgbfs_library_keys_to_aos_v1(KeysV1 { rows: 3, ..target }, output, 4, stream),
            0
        );
        ...
        for plane in target.words {
            let mut guard = 0u32;
            assert_eq!(
                cudaMemcpy((&mut guard as *mut u32).cast(), plane.add(3).cast(), 4, 2),
                0
            );
            assert_eq!(guard, 99);
        }
        ...
        for buffer in [old_history, input, scratch, output] {
