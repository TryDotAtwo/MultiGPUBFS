...
pub struct DistributedNativeBfs {
    ...
    stream:Stream,archive_stream:Stream,archive_done:[Event;2],current_bank:usize,archived_depth:Option<u32>,comm:Comm,generate:Plan,hash:Plan,archive_hash:Plan,archive_hashes:Buffer,route:Plan,merge:Plan,settle:Plan,materialize:Plan,
...
impl DistributedNativeBfs {
    pub fn new(graph:&MatrixGroup,seed:[u8;16],id:[u8;128],cfg:DistributedConfig)->Result<Self>{
        ...
        let archive_hash=Plan::new(mgbfs_hash_destroy,|out,e|unsafe{mgbfs_hash_create(width as u32,cfg.batch,limbs.as_ptr(),contract.offsets.as_ptr(),out,e,512)})?;
        ...
        let result=Self{cfg,width,stride,moves,candidates,depth:0,current_count,failed:false,stream,archive_stream,archive_done,current_bank:0,archived_depth:None,comm,generate,hash,archive_hash,archive_hashes:Buffer::new(cfg.batch as usize*16,raw_archive)?,route,merge,settle,materialize,
    ...
    fn produce(&mut self)->Result<()> {
        self.future_state.put(&[FrontierState::default()])?;let mut offset=0u32;
        loop {
            ...
            offset=offset.saturating_add(parents);let more=(offset<self.current_count) as u32;if self.all_max(more)?==0{break}
        } Ok(())
    }
    ...
    pub fn archive_current(&mut self,archive:&mut crate::pinned_archive::PinnedArchive)->Result<()>{if self.failed{return Err("DISTRIBUTED_FAILED".into())}if archive.width!=self.width{return Err("ARCHIVE_STATE_WIDTH".into())}if self.archived_depth==Some(self.depth){return Err("ARCHIVE_DEPTH_ALREADY_SUBMITTED".into())}let s=self.archive_stream.0;let mut offset=0u32;while offset<self.current_count{let n=archive.rows.min(self.cfg.batch).min(self.current_count-offset);let slot=archive.acquire()?;let copied=(||unsafe{let states=self.current_states.at(offset as usize*self.stride);check(mgbfs_hash_run(self.archive_hash.0,states.cast(),self.archive_hashes.ptr.cast(),n,s))?;check(cudaMemcpy2DAsync(slot.ptr,self.width,states,self.stride,self.width,n as usize,2,s))?;check(cudaMemcpyAsync(slot.ptr.cast::<u8>().add(n as usize*self.width).cast(),self.archive_hashes.ptr,n as usize*16,2,s))?;check(cudaEventRecord(slot.ready,s))})();if let Err(e)=copied{unsafe{cudaStreamSynchronize(s);}self.failed=true;return Err(e)}archive.submit(slot,u64::from(self.depth),n)?;offset+=n}check(unsafe{cudaEventRecord(self.archive_done[self.current_bank].0,s)})?;archive.layer(u64::from(self.depth),u64::from(self.current_count))?;self.archived_depth=Some(self.depth);Ok(())}
