...
pub struct DistributedNativeBfs {
    ...
    stream:Stream,comm:Comm,generate:Plan,hash:Plan,route:Plan,merge:Plan,settle:Plan,materialize:Plan,
...
impl DistributedNativeBfs {
    pub fn new(graph:&MatrixGroup,seed:[u8;16],id:[u8;128],cfg:DistributedConfig)->Result<Self>{
        ...
        Ok(Self{cfg,width,stride,moves,candidates,depth:0,current_count,failed:false,stream,comm,generate,hash,route,merge,settle,materialize,
    ...
    fn advance_inner(&mut self)->Result<bool>{self.produce()?;let target=self.depth.checked_add(1).ok_or("DEPTH_OVERFLOW")?;let count=self.settle(target)?;self.depth=target;std::mem::swap(&mut self.current_states,&mut self.next_states);self.current_count=count;Ok(self.all_max((count>0) as u32)?!=0)}
