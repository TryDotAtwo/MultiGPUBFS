...
extern "C" int mgbfs_state_build_requests(const MgbfsRegenerateOrigin*,uint32_t,
 const uint64_t*,uint32_t,const uint32_t*,uint32_t,MgbfsRegenerateOrigin*,uint64_t*,
 uint32_t*,MgbfsStateRingControl*,MgbfsOwnerControl*,MgbfsStateExtent*,void*);
...
int main(){
 ...
 MgbfsStateRingControl ring{};ring.tail=7;ring.capacity=16;ring.descriptor_capacity=8;ring.descriptor_tail=1;
 MgbfsOwnerControl owner{};owner.stage=2;owner.survivors=2;
 MgbfsStateExtent extent{};extent.sequence=5;extent.begin=5;extent.count=2;extent.granted_rows=2;
 ...
 require(dest[0]==5&&dest[1]==6);
 require(cudaMemcpy(&extent,e,sizeof(extent),cudaMemcpyDeviceToHost)==cudaSuccess);
 require(extent.ready==0&&extent.sequence==5&&extent.count==2); // Commit is final, states not ready.
