...
Commit `edc738b` added a CPU-tested `AdmittedBuffers` gate: `FinalizeDepth` ACK requires settled macro history and a free replacement slot; admitted `Publish` advances the history window. This is control-plane integration, not a complete weighted BFS. Kaggle 2xT4 version 9 is running at source `edc738be1498fe2b1b1bd5dca881a88f80da5ceb`; physical result pending. Full CUDA event binding and end-to-end weighted scheduler remain open.
